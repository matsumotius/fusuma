(function($) {
  ...
  Fusuma.prototype.add = function(content) {
    ...
    return {
      to : function(dir){
        if(that.contents.length < 1){
          $(that.target).find('ul').append(Tag.generate('li', {}, content)); 
        } else {
          $(that.contents[POINT[dir]])[INSERT_TO[dir]](Tag.generate('li', {}, content));
        }
        if(that.current < 0 || dir === 'start' || dir === 'prev') that.current++;
        that.apply_contents();
      }
    };
  };
  ...
  Fusuma.prototype.go = function(dir, callback) {
    var dispatch_mapping = {
      next    : 'slide_into_right',
      forward : 'slide_into_right',
      prev    : 'slide_into_left',
      back    : 'slide_into_left'
    };
  ...
  Fusuma.prototype.slide_into_right = function(callback) {
    ...
    $(this.target).filter(':not(:animated)').animate({
      ...
      complete : function(){ 
        ...
        if(_this.browser_is('Firefox') ||
           _this.browser_is('Opera')) $(_this.target).scrollLeft(_this.current * _this.width);
      }
    });
  };
